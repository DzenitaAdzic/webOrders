...
public class KorisnikFacadeREST extends AbstractFacade<Korisnik> {
    ...
    @Path("/naziv/{username}")
    @Produces(MediaType.APPLICATION_JSON)
    public Korisnik findByNaziv(@PathParam("username")String username){
    
       Korisnik f = (Korisnik) em.createNamedQuery("Korisnik.findByUsername").setParameter("username", username).getSingleResult();
